...
export async function sendInviteEmail(
  ...
  const wsLabel = workspaceName ? ` ${workspaceName} on` : "";

  await getResend().emails.send({
    from: process.env.RESEND_FROM_EMAIL || "Shronk <noreply@shronk.com>",
    to: email,
    subject: `${inviterName} invited you to${wsLabel} Shronk`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2>You've been invited to Shronk</h2>
        <p>${inviterName} has invited you to join${workspaceName ? ` <strong>${workspaceName}</strong> on` : ""} Shronk.</p>
        <p>
